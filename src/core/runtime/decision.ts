...
export function highestDecision(answer: Decision, legal: string[]): Decision {
  ...
  if (
    ...
    Math.abs(Object.values(p).reduce((a, b) => a + b, 0) - 1) > 0.025
  )
    throw new DecisionError("Invalid Jev choice or probability distribution.");
  // Preserve Jev's selected choice on ties; otherwise use the highest probability.
  const choice = legal.reduce(
    (best, id) => (p[id]! > p[best]! ? id : best),
    answer.choice,
  );
