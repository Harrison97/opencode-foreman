...
const strings = { type: 'array', items: string, uniqueItems: true, maxItems: 100 };
const outcomes = { type: 'object', additionalProperties: false, properties: { ready: strings, incomplete: strings, blocked: strings } };
const schema = {
  type: 'object', additionalProperties: false, required: ['version', 'name', 'admission', 'capabilities'],
  properties: {
    ...
    capabilities: { type: 'object', minProperties: 1, maxProperties: 100,
      ...
      additionalProperties: {
        type: 'object', additionalProperties: false, required: ['purpose', 'instructions', 'completion'],
        properties: {
...
export function validateOutput(schema: Record<string, unknown>, data: Record<string, unknown>): void {
  const check = ajv.compile(schema);
  if (!check(data)) throw new Error('Invalid capability output: ' + ajv.errorsText(check.errors));
}
export function parseWorkflow(value: unknown): Workflow {
  ...
  for (const id of ids) {
    if (['constructor', 'prototype', '__proto__', 'BYPASS'].includes(id)) throw new Error('Reserved capability identifier');
    const c = w.capabilities[id]!;
    if (c.model) parseModel(c.model);
    if (c.outputs) {
      if (c.outputs.type !== 'object') throw new Error('Capability outputs schema must describe an object');
  ...
  for (const id of reached) for (const next of Object.values(w.capabilities[id]!.next ?? {}).flat()) reached.add(next);
  if (ids.some(id => !reached.has(id))) throw new Error('Unreachable capability');
  if (!ids.some(id => w.capabilities[id]!.terminal)) throw new Error('Workflow requires a terminal delivery capability');
