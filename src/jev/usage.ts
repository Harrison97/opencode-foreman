...
export interface JevUsage {
  ...
  model: string | null;
  status: 'pending' | 'success' | 'http_error' | 'invalid_response' | 'transport_error';
...
export class UsageLog {
  ...
  constructor(readonly root: string) { this.path = join(root, '.jev', 'usage.jsonl'); }
  ...
  async read(): Promise<{ records: JevUsage[]; unreadableLines: number }> {
    ...
    catch (e) { if ((e as NodeJS.ErrnoException).code === 'ENOENT') return { records: [], unreadableLines: 0 }; throw e; }
    const records: JevUsage[] = [];
    let unreadableLines = 0;
    for (const line of text.split('\n').filter(Boolean)) {
      try {
        const value = JSON.parse(line) as JevUsage;
        if (value.schema !== 1 || typeof value.requestID !== 'string' ||
            !['pending', 'success', 'http_error', 'invalid_response', 'transport_error'].includes(value.status)) throw new Error('Invalid usage record');
        records.push(value);
...
export function summarizeUsage(records: JevUsage[]) {
  ...
  const totals = (items: JevUsage[]) => ({
    ...
    failedRequests: items.filter(r => r.status !== 'success' && r.status !== 'pending').length,
    inputTokens: items.reduce((sum, r) => sum + (tokenCount(r.inputTokens) ?? 0), 0),
    outputTokens: items.reduce((sum, r) => sum + (tokenCount(r.outputTokens) ?? 0), 0),
    unknownInputRequests: items.filter(r => tokenCount(r.inputTokens) === null).length,
    unknownOutputRequests: items.filter(r => tokenCount(r.outputTokens) === null).length,
  });
  const models = [...new Set(requests.map(r => r.model))].map(model => ({
    model, requestedModels: [...new Set(requests.filter(r => r.model === model).map(r => r.requestedModel))],
    ...totals(requests.filter(r => r.model === model)),
  }));
  const sessions = [...new Set(requests.map(r => r.sessionID))].map(sessionID => ({
    sessionID, ...totals(requests.filter(r => r.sessionID === sessionID)),
  }));
