...
export async function loadWorkflowConfig(directory: string) {
  ...
  try {
    ...
    throw new Error('Legacy jev.workflow.json is model-only. Migrate to jev.workflow.yaml; see docs/workflows.md.');
  } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; }
  return loadWorkflowFile(fileURLToPath(new URL('../workflows/software-engineer/workflow.yaml', import.meta.url)));
}
