...
test('conflicting shared output types fail; tool overlap and intentional pauses warn', () => {
  const w = structuredClone(sample);
  w.capabilities.proof!.outputs = {type:'object',properties:{labels:{type:'number'}}};
  assert.throws(() => parseWorkflow(w), /Incompatible shared-field types/);
  ...
  assert.ok(warnings.some(d => d.path.endsWith('next.incomplete') && d.message.includes('pauses')));
...
test('CLI reports warnings, rejects bad contracts, and optionally queries host inventory', async () => {
  ...
  const server = createServer((req, res) => {
    ...
    res.end(JSON.stringify(path === '/provider' ? {all:[],connected:[]} : ['read','bash']));
  });
  await new Promise<void>(resolve => server.listen(0, '127.0.0.1', resolve));
  try {
    const address = server.address() as {port:number};
    const result = await run(process.execPath, ['--import','tsx','scripts/check-workflow.ts',file,'--host',`http://127.0.0.1:${address.port}`]);
