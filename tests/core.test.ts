...
test('custom names, dependencies, legal transitions, fallback, and terminal evidence', async () => {
  ...
  await assert.rejects(f.c.report('s', { summary: 'done', outcome: 'ready', covered: ['Correct tone'] }), /native evidence/);
  await f.c.evidence('s', { callID: 'a', command: 'node --test', exit: 0, output: 'passed' });
  await f.c.report('s', { summary: 'proofread', outcome: 'ready', covered: ['Correct tone'] });
  const end = await f.c.gate('s','msg_proof');
  assert.equal(end?.status, 'complete'); assert.equal(end?.capability, 'publish');
...
test('rejected output and coverage reports are atomic and corrected without rerunning checks', async () => {
  const f = await fixture(); const before = await f.state();
  await assert.rejects(f.c.report('s', { summary:'done', outcome:'ready', data: { labels: [] } }), /output/);
  assert.deepEqual(await f.state(), before);
  await advance(f.c);
  await f.c.evidence('s', { callID:'a', command:'node --test', exit:0, output:'ok' });
  await assert.rejects(f.c.report('s', { summary:'done', outcome:'ready', covered:['paraphrased'] }), /do not need to be rerun/);
  await assert.rejects(f.c.report('s', { summary:'done', outcome:'ready', data:{checks:['true']} }), /Cannot edit/);
  await f.c.report('s', { summary:'done', outcome:'ready', covered:['Correct tone'] });
  assert.equal((await f.c.gate('s','done'))?.status, 'complete');
});
test('failed verification routes backward and requires fresh evidence after repair', async () => {
  const f = await fixture(); await advance(f.c);
  await f.c.evidence('s', { callID:'a',command:'node --test',exit:0,output:'pass' });
  await f.c.evidence('s', { callID:'b',command:'node --test',exit:1,output:'fail' });
  await assert.rejects(f.c.report('s', {summary:'done',outcome:'ready',covered:['Correct tone']}), /native evidence/);
  await f.c.report('s', {summary:'typo found',outcome:'incomplete'});
  const back = await f.c.gate('s','failed');
  assert.equal(back?.capability,'draft'); assert.deepEqual(back?.completed,{});
  await f.c.admit('s',back!.pending!.text,back!.pending!.id,true);
  await advance(f.c,'s','repair');
  await assert.rejects(f.c.report('s',{summary:'done',outcome:'ready',covered:['Correct tone']}),/native evidence/);
  await f.c.evidence('s',{callID:'c',command:'node --test',exit:0,output:'fixed'});
  await f.c.report('s',{summary:'done',outcome:'ready',covered:['Correct tone']});
  assert.equal((await f.c.gate('s','verified'))?.status,'complete');
});
test('tool filters and exact command evidence; duplicate calls and null exits', async () => {
  ...
  await f.c.evidence('s',{callID:'unrelated',command:'true',exit:0,output:''});
  await f.c.evidence('s',{callID:'x',command:'node --test',exit:null,output:'timeout'});
  await f.c.evidence('s',{callID:'x',command:'node --test',exit:0,output:'fake second'});
...
test('human pause/resume and reload keep the pinned workflow; synthetic messages do not resume', async () => {
  const f = await fixture();
  await f.c.report('s',{summary:'Need audience',outcome:'blocked',questions:['Who is the audience?']});
  ...
  const edited = structuredClone(sample); edited.capabilities.draft!.instructions = 'NEW RULE';
  const originalID = (await f.state()).id;
  const reload = new Controller(new StateStore(f.root),chooser(),{workflow:edited});
  const resumed = await reload.admit('new','foreman resume: Internal engineers');
  assert.equal(resumed?.id,originalID);
  assert.equal(resumed?.status,'running');
  assert.equal(resumed?.workflow.capabilities.draft!.instructions,sample.capabilities.draft!.instructions);
});
test('internal prompts and repeated idle events cannot recursively admit', async () => {
  const f = await fixture(); const next = await advance(f.c);
  assert.equal(await f.c.gate('s','msg_draft'),undefined);
  assert.equal((await f.state()).pending?.delivered,true);
...
test('finite work limits and repeated work pause without special workflow nodes', async () => {
  const f = await fixture();
  const c = new Controller(f.store,chooser(),{workflow:sample,maxTurns:1});
  await c.report('s',ready); assert.equal((await c.gate('s','budget'))?.status,'paused');
...
test('durable state, locking, old state preservation and secret redaction', async () => {
  const f = await fixture(); const other = new StateStore(f.root);
  const oldPath=join(f.root,'.jev/state.json');
  await writeFile(oldPath,'{"schema":1,"historical":"untouched"}');
  await Promise.all(Array.from({length:12},(_,i)=>(i%2?f.store:other).transaction(db=>{db.workflows[db.active!]!.revision++;})));
  assert.equal((await f.state()).revision,12);
  assert.equal(await readFile(oldPath,'utf8'),'{"schema":1,"historical":"untouched"}');
  assert.equal((await stat(f.store.path)).mode & 0o777,0o600);
  process.env.FOREMAN_TEST_SECRET='test-secret-value-123456';
  try { await f.c.report('s',{summary:process.env.FOREMAN_TEST_SECRET,outcome:'incomplete'}); assert.ok(!(await readFile(f.store.path,'utf8')).includes(process.env.FOREMAN_TEST_SECRET)); }
  finally { delete process.env.FOREMAN_TEST_SECRET; }
...
test('a completed workflow detaches before normal conversation or a fresh request',async()=>{
  const f=await fixture();await advance(f.c);
  await f.c.evidence('s',{callID:'pass',command:'node --test',exit:0,output:'pass'});
  await f.c.report('s',{summary:'checked',outcome:'ready',covered:['Correct tone']});
  await f.c.gate('s','finished');
  const bypass = new Controller(f.store,chooser('BYPASS'),{workflow:sample});
  assert.equal(await bypass.admit('s','What does this mean?'),undefined);
  assert.equal(await bypass.get('s'),undefined);
  assert.equal((await f.c.admit('s','foreman: Write another draft'))?.capability,'draft');
