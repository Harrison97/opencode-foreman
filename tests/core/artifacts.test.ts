...
function workflow() {
  ...
  (
    w.capabilities.draft!.outputs!.properties as Record<string, unknown>
  ).artifacts = {
    type: "array",
    minItems: 1,
...
test("artifact paths are checked before accepting the producer report and can differ on each visit", async () => {
  ...
  const report = (name: string) => ({
    ...ready,
    data: { ...ready.data, artifacts: [name] },
  });
...
test("artifact references check the named producer and prevent outside-project paths and symlinks", async () => {
  ...
  await assert.rejects(
    f.c.report("s", {
      ...ready,
      data: { ...ready.data, artifacts: ["outside"] },
    }),
    /outside project/,
  );
  ...
  await f.c.report("s", {
    ...ready,
    data: { ...ready.data, artifacts: ["draft.md"] },
  });
  const next = await f.c.gate("s", "ready");
  await f.c.admit("s", "continue", next!.pending!.id, true);
...
test("checker rejects legacy gate names and invalid artifact sources; acceptance can stand alone", async () => {
  for (const gate of [
    { checks: "draft.checks" },
    { coverage: "draft.labels" },
    { files: "missing.artifacts" },
    { files: "artifacts" },
  ]) {
    const w = structuredClone(sample);
    (w.capabilities.proof as any).gate = gate;
    assert.throws(() => parseWorkflow(w));
  }
  const w = structuredClone(sample);
  w.capabilities.proof!.gate = { acceptance: "draft.labels" };
  delete w.capabilities.proof!.tools;
  const f = await fixture(w);
  await advance(f.c);
  await assert.rejects(
    f.c.report("s", { summary: "reviewed", outcome: "ready" }),
