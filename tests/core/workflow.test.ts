...
test("local workflow repo imports capabilities, markdown instructions and JSON schemas", async () => {
  ...
  draft.instructions = { file: "prompt.md" };
  draft.outputs = { file: "output.json" };
  await writeFile(
    join(repo, "library.yaml"),
    YAML.stringify({ capabilities: { draft } }),
  );
  delete w.capabilities.draft;
  ...
  assert.equal(
    loaded.capabilities.draft!.instructions,
    "Custom editorial guidance.",
  );
  assert.deepEqual(
    loaded.capabilities.draft!.outputs,
    sample.capabilities.draft!.outputs,
  );
});
test("invalid YAML, conflicting imports, symlink escapes, and missing sources fail explicitly", async () => {
  ...
  await writeFile(
    join(root, "lib.yaml"),
    YAML.stringify({ capabilities: { draft: sample.capabilities.draft } }),
  );
  ...
  await writeFile(
    join(root, "lib.yaml"),
    YAML.stringify({ imports: ["lib.yaml"], capabilities: {} }),
  );
  ...
  const w: any = structuredClone(sample);
  w.capabilities.draft.instructions = { file: "prompt.md" };
...
test("required artifacts must exist; append-only contract labels survive repair", async () => {
  const w = structuredClone(sample);
  w.capabilities.draft!.gate = { files: ["DRAFT.md"] };
  ...
  const back = await f.c.gate("s", "back");
  await f.c.admit("s", "continue", back!.pending!.id, true);
  await f.c.report("s", {
    ...ready,
    data: { ...ready.data, labels: ["New criterion"] },
  });
  assert.deepEqual((await f.state()).capabilityOutputs.draft!.labels, [
    "Correct tone",
    "New criterion",
  ]);
});
test("capability model overrides host model and persists attribution", async () => {
  assert.deepEqual(parseModel("provider/model"), {
    providerID: "provider",
    modelID: "model",
  });
  assert.throws(() => parseModel("invalid"));
  const w = structuredClone(sample);
  w.capabilities.proof!.model = "provider/reviewer";
  const f = await fixture(w);
  await f.c.admit("s", "continue", undefined, false, {
    model: { providerID: "provider", modelID: "default" },
  });
