...
import { UsageLog, summarizeUsage, type JevUsage } from '../src/jev/usage.js';
import { StateStore } from '../src/core/state.js';
import { Controller } from '../src/core/controller.js';
import { execFileSync } from 'node:child_process';
import { sample, ready } from './fixtures.js';

const response = (extras: Record<string, unknown> = {}) => ({
  model: 'jev-1.13.0', usage: { input_tokens: 123, output_tokens: 12 },
  answers: { next: { type: 'choice', choice: 'IMPLEMENT', confidence: 0.2, probabilities: { IMPLEMENT: 1 } } }, ...extras,
});
...
test('persists actual model and exact tokens even for low-confidence decisions; counts a request once', async () => {
  ...
  assert.equal(summary.requests, 1); assert.equal(summary.inputTokens, 123); assert.equal(summary.outputTokens, 12);
  assert.equal(summary.models[0]!.model, 'jev-1.13.0'); assert.equal(summary.unknownInputRequests, 0);
  assert.equal(requestBody.sessionID, undefined);
  const text = await readFile(f.log.path, 'utf8');
  assert.ok(!text.includes('private goal')); assert.ok(!text.includes('fake-test-key-not-real'));
...
test('missing or malformed token usage stays unknown and does not break valid decisions', async () => {
  for (const usage of [undefined, { input_tokens: -2, output_tokens: 1.5 }, { input_tokens: '123', output_tokens: null }]) {
    ...
    assert.equal(summary.unknownInputRequests, 1); assert.equal(summary.unknownOutputRequests, 1);
  }
});
test('HTTP failures and network interruptions are recorded without response bodies or credentials', async () => {
  for (const [fetcher, status] of [
    [async () => new Response('private provider error text', { status: 429 }), 'http_error'],
    [async () => { throw new Error('private transport error text'); }, 'transport_error'],
  ] as const) {
...
test('unparseable JSON records an invalid response with unknown usage', async () => {
  const f = await fixture(async () => new Response('broken JSON', { status: 200 }));
  await assert.rejects(f.call());
...
test('concurrent requests preserve separate session and model totals across reload', async () => {
  ...
  await Promise.all(Array.from({ length: 8 }, (_, i) => f.client.choose({}, { IMPLEMENT: 'implement' }, 'next', { sessionID: i % 2 ? 'ses_a' : 'ses_b' })));
  const summary = summarizeUsage((await new UsageLog(f.root).read()).records);
  assert.equal(summary.requests, 8); assert.equal(summary.inputTokens, 8 * 123);
  assert.equal(summary.sessions.length, 2); assert.ok(summary.sessions.every(s => s.requests === 4));
});
test('interrupted requests and damaged ledger lines are visible', async () => {
  const f = await fixture(async () => Response.json(response())); await f.call();
  const records = (await f.log.read()).records;
  const interrupted: JevUsage = { ...records[0]!, requestID: 'interrupted' };
  await f.log.append(interrupted); await appendFile(f.log.path, '{broken\n');
  const result = await f.log.read(); const summary = summarizeUsage(result.records);
  assert.equal(result.unreadableLines, 1); assert.equal(summary.requests, 2); assert.equal(summary.pendingRequests, 1);
  assert.equal(summary.inputTokens, 123); assert.equal(summary.unknownInputRequests, 1);
});
test('admission and workflow gates both attribute usage without recursively locking state', async () => {
  const f = await fixture(async (_url, init) => {
    ...
    return Response.json(response({ answers: { next: { type: 'choice', choice, confidence: 1,
      probabilities: Object.fromEntries(Object.keys(body.questions.next.criteria).map(k => [k, k === choice ? 1 : 0])) } } }));
  });
  const controller = new Controller(new StateStore(f.root), f.client, { workflow: sample });
  await controller.admit('ses_attributed', 'foreman: Write a handbook.');
  await controller.report('ses_attributed', ready);
  assert.equal((await controller.gate('ses_attributed', 'msg_done'))?.capability, 'proof');
  const summary = summarizeUsage((await f.log.read()).records);
  assert.equal(summary.requests, 2); assert.equal(summary.sessions[0]!.sessionID, 'ses_attributed');
});
...
test('usage summary command reports recorded tokens and supports session filtering', async () => {
  const f = await fixture(async () => Response.json(response())); await f.call();
  const run = (session?: string) => JSON.parse(execFileSync(process.execPath,
    ['--import', 'tsx', 'scripts/jev-usage.ts', f.root, ...(session ? [session] : [])], { encoding: 'utf8' }));
  const summary = run();
  assert.equal(summary.requests, 1); assert.equal(summary.inputTokens, 123); assert.equal(summary.complete, true);
